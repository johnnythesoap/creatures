...
public class GameTestRegistry {
    ...
    public static void register(Method pTestMethod) {
        register(pTestMethod, java.util.Set.of());
    }

    /** @deprecated Forge: Use {@link net.minecraftforge.event.RegisterGameTestsEvent RegisterGameTestsEvent} to register game tests */
    @Deprecated
    public static void register(Method pTestMethod, java.util.Set<String> allowedNamespaces) {
        String s = pTestMethod.getDeclaringClass().getSimpleName();
        GameTest gametest = pTestMethod.getAnnotation(GameTest.class);
        if (gametest != null) {
            net.minecraftforge.gametest.ForgeGameTestHooks.addTest(TEST_FUNCTIONS, TEST_CLASS_NAMES, allowedNamespaces, turnMethodIntoTestFunction(pTestMethod));
        }

        GameTestGenerator gametestgenerator = pTestMethod.getAnnotation(GameTestGenerator.class);
        if (gametestgenerator != null) {
            for (var func : useTestGeneratorMethod(pTestMethod)) {
